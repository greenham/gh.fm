...
const commands = {
	'play': (msg) => {
		...
		(function play(song) {
			...
			collector.on('message', m => {
				if (m.content.startsWith(tokens.prefix + 'pause') && (m.author.username === song.requester || m.author.id === tokens.adminID)) {
					msg.channel.sendMessage('paused').then(() => {dispatcher.pause();});
				} else if (m.content.startsWith(tokens.prefix + 'resume') && (m.author.username === song.requester || m.author.id === tokens.adminID)){
	...
	// @todo add a way to remove the most recent song added to the queue, or remove by #?
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Add some songs to the queue first with ${tokens.prefix}add`);
		let tosend = [];
...
function dmUser(originalMessage, newMessage)
{
  ...
  } else if (originalMessage.member !== undefined) {
